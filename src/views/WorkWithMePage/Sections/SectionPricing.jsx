						...
						<GridContainer>
							<GridItem xs={12} style={{ textAlign: 'center' }}>
								...
								<h5 className={classes.description}>
									When reviewing the following packages, note that my typical client loses at least
									$1,000+ every week they are unemployed (this does not include health insurance &
									other benefits). When you take into account the $5,000 - $10,000 salary increases
									clients often receive with their new job offer, your package is a small investment
							...
							<GridItem xs={12} style={{ textAlign: 'center' }}>
								<Table
									...
									customCellClasses={[ classes.textLeft, classes.textRight, classes.textRight ]}
							...
							<GridItem xs={12} className={classes.description}>
								...
								<ul>
									{add_on_pricing.map((item, idx) => (
										<div key={idx}>
											<li>
												<strong>{item.name}</strong> {item.price ? `- ${item.price}` : <div />}
											</li>
							...
							<GridItem xs={12} className={classes.description}>
								...
								<strong>motivation</strong> and most importantly, <strong>confidence</strong> in your
								job search.
							</GridItem>
							<GridItem xs={12}>
								<h5
